...
async def get_available_slots(
    ...
    """
    try:
        slots = await interview_service.get_available_slots(db, data.panelist_ids, data.start_date, data.end_date)
        return slots
...
async def schedule_interview(
    ...
    """
    try:
        interview = await interview_service.schedule_interview(db, data)
        return interview
...
async def update_interview_status(
    ...
    """
    try:
        interview = await interview_service.update_interview_status(db, interview_id, status)
        return interview
    except ValueError as e:
