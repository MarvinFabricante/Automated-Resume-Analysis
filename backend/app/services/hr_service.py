...
class HRService:
    async def create_hr_profile(self, db: AsyncSession, hr_in: HRCreate):
    ...
    async def get_hr_profile(self, db: AsyncSession, hr_id: int):
        return await HRRepository.get_hr_by_id(db, hr_id)

    async def update_hr_profile(self, db: AsyncSession, hr_id: int, hr_update: HRUpdate):
    ...
    async def get_total_candidates_count(self, db: AsyncSession):
        return await HRRepository.get_total_candidates_count(db)

    async def get_total_resumes_count(self, db: AsyncSession):
        return await HRRepository.get_total_resumes_count(db)

    async def get_application_stats(self, db: AsyncSession):
    ...
    async def get_dashboard_trends(self, db: AsyncSession):
...
async def get_hr_profile(db: AsyncSession, hr_id: int):
    return await hr_service.get_hr_profile(db, hr_id)


async def update_hr_profile(db: AsyncSession, hr_id: int, hr_update: HRUpdate):
    return await hr_service.update_hr_profile(db, hr_id, hr_update)


async def get_total_candidates_count(db: AsyncSession):
    return await hr_service.get_total_candidates_count(db)


async def get_total_resumes_count(db: AsyncSession):
    return await hr_service.get_total_resumes_count(db)


async def get_application_stats(db: AsyncSession):
    return await hr_service.get_application_stats(db)
