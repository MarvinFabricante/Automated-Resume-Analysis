...
class JobService:
    async def create_job(self, db: AsyncSession, job: JobCreate):
    ...
    async def get_job(self, db: AsyncSession, job_id: str):
        """Get a job by job_id."""
        return await JobDescriptionRepository.get_by_job_id(db, job_id)

    async def get_all_active_jobs(self, db: AsyncSession, skip: int = 0, limit: int = 100, include_inactive: bool = False):
        """Get all active jobs with pagination."""
        return await JobDescriptionRepository.get_all_active(db, skip, limit, include_inactive)

    async def update_job(self, db: AsyncSession, job_id: str, job_data: JobUpdate):
    ...
    async def set_job_status(self, db: AsyncSession, job_id: str, active_status: bool):
    ...
    async def delete_job(self, db: AsyncSession, job_id: str):
        """Delete a job."""
        db_job = await JobDescriptionRepository.get_by_job_id(db, job_id)
...
async def get_all_active_jobs(db: AsyncSession, skip: int = 0, limit: int = 100, include_inactive: bool = False):
    return await job_service.get_all_active_jobs(db, skip, limit, include_inactive)
...
async def set_job_status(db: AsyncSession, job_id: str, active_status: bool):
    return await job_service.set_job_status(db, job_id, active_status)
